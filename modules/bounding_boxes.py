		...
		if args.multiclasses == '0':
			...
			for classes in args.classes:
				...
				if args.type_csv == 'train':
					...
					if not args.n_threads:
						download(args, df_val, folder[0], dataset_dir, class_name, class_code, df_classes)
					else:
						download(args, df_val, folder[0], dataset_dir, class_name, class_code, df_classes, threads = int(args.n_threads))

				elif args.type_csv == 'validation':
					name_file = file_list[1]
					df_val = TTV(csv_dir, name_file, args.yes)
					if not args.n_threads:
						download(args, df_val, folder[1], dataset_dir, class_name, class_code, df_classes)
					else:
						download(args, df_val, folder[1], dataset_dir, class_name, class_code, df_classes, threads = int(args.n_threads))

				elif args.type_csv == 'test':
					name_file = file_list[2]
					df_val = TTV(csv_dir, name_file, args.yes)
					if not args.n_threads:
						download(args, df_val, folder[2], dataset_dir, class_name, class_code, df_classes)
					else:
						download(args, df_val, folder[2], dataset_dir, class_name, class_code, df_classes, threads = int(args.n_threads))
					...
					for i in range(3):
						...
						if not args.n_threads:
							...
							download(args, df_val, folder[i], dataset_dir, class_name, class_code, df_classes, threads = int(args.n_threads))
				else:
			...
			for class_name in class_list:
				...
				if args.type_csv == 'train':
					...
					df_val = TTV(csv_dir, name_file, args.yes)
					if not args.n_threads:
						download(args, df_val, folder[0], dataset_dir, class_name, class_dict[class_name], class_list, df_classes)
					else:
						download(args, df_val, folder[0], dataset_dir, class_name, class_dict[class_name], class_list, df_classes, int(args.n_threads))

				elif args.type_csv == 'validation':
					name_file = file_list[1]
					df_val = TTV(csv_dir, name_file, args.yes)
					if not args.n_threads:
						download(args, df_val, folder[1], dataset_dir, class_name, class_dict[class_name], class_list, df_classes)
					else:
						download(args, df_val, folder[0], dataset_dir, class_name, class_dict[class_name], class_list, df_classes, int(args.n_threads))
					...
					df_val = TTV(csv_dir, name_file, args.yes)
					if not args.n_threads:
						download(args, df_val, folder[2], dataset_dir, class_name, class_dict[class_name], class_list, df_classes)
					else:
						download(args, df_val, folder[0], dataset_dir, class_name, class_dict[class_name], class_list, df_classes, int(args.n_threads))
					...
					for i in range(3):
						...
						df_val = TTV(csv_dir, name_file, args.yes)
						if not args.n_threads:
							download(args, df_val, folder[i], dataset_dir, class_name, class_dict[class_name], class_list, df_classes)
						else:
							download(args, df_val, folder[0], dataset_dir, class_name, class_dict[class_name], class_list, df_classes, int(args.n_threads))
